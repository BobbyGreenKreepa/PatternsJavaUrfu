...
## Задание 22
Создание State в [`contentstate`](./src/main/java/com/example/javapatternsproject/common/usecase/contentstate)
### Причина выбора
Позволяет удобно реагировать на разные возможные состояние контента.
### Признаки паттерна
Состояния вынесены в отдельные классы EmptyState NoContentState HeaderOnlyState и реализуют различное поведение для объекта Content.
Класс Content не реализует логику самостоятельно, а передаёт вызовы методов (getText и getInners) текущему состоянию,
## Задание 23
Создание Strategy в [`OnErrorStrategy`](./src/main/java/com/example/javapatternsproject/common/request/OnErrorStrategy.java)
### Причина выбора
Паттерн стратегия хорошо вписывается в обработку ошибок, и позволяет гибко настраивать фасад для разных кейсов
### Признаки паттерна
Описанный интерфейс для стратегии
Класс RequestFacade делелгирует логику обработки ошибок стратегии которая задается из вне.
